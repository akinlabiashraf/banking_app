...
<head>
  ...
  <title>BloomSpring - User Dashboard</title>
...
<body x-data class="is-header-blur" x-bind="$store.global.documentBody">
  ...
  <div
        ...
        <div
          ...
          <div
            ...
            <a
              ...
              x-tooltip.placement.right="'My Account'">
              <svg
                class="size-7"
      ...
      <div
        ...
        <div class="flex w-full items-center justify-between">
          ...
          <div class="-mr-1.5 flex items-center space-x-2">
            ...
            <h1><?php echo "Welcome to BloomSpring " . $username . " !!!"; ?></h1>
    ...
    $connection = new mysqli("localhost", "root", "", "banking_app");
    ...
    try {
      ...
      $balanceQuery = $connection->prepare("SELECT `balance` FROM `accounts` WHERE `user_id` = ?");
      ...
      $incomeQuery = $connection->prepare("SELECT SUM(`amount`) AS total_income FROM `transactions` WHERE `transaction_type` = 'income' AND `receiver_id` = ?");
      ...
      $expenseQuery = $connection->prepare("SELECT SUM(`amount`) AS total_expense FROM `transactions` WHERE `transaction_type` = 'expense' AND `sender_id` = ?");
      ...
      <div
        ...
        <div
          ...
          <div class="col-span-12 sm:col-span-6 lg:col-span-4">
            <div class="px-4 text-white sm:px-5">
              ...
              <div class="mt-3">
                <!-- Display the overall balance here -->
                <p class="text-2xl font-semibold">$<?= number_format($balance, 2) ?></p> <!-- Overall balance -->
              </div>

              <div class="mt-4 flex space-x-7">
                <div>
                  <p class="text-indigo-100">Weekly Income</p>
                  <div class="mt-1 flex items-center space-x-2">
                    ...
                    <!-- <p class="text-base font-medium">$<?= number_format($weeklyIncome, 2) ?></p> Weekly income -->
                  </div>
                </div>
                <div>
                  <p class="text-indigo-100">Weekly Expense</p>
                  <div class="mt-1 flex items-center space-x-2">
                    <div class="flex size-7 items-center justify-center rounded-full bg-black/20">
                      <svg xmlns="http://www.w3.org/2000/svg" class="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <!-- <p class="text-base font-medium">$<?= number_format($weeklyExpense, 2) ?></p> Weekly expense -->
                  </div>
                </div>
              </div>
            </div>
        ...
        <div class="card group col-span-12 pb-5 lg:col-span-8">
          ...
          <div
            ...
            <div class="col-span-12 rounded-lg bg-slate-50 p-3 dark:bg-navy-600 sm:col-span-12 xl:col-span-12">
              <div class="space-y-4">
                <?php
                ...
                $sql = "SELECT t.transaction_id, t.amount, t.transaction_type, t.description, t.transaction_date, u.username
                FROM transactions t
                JOIN users u ON t.sender_id || t.receiver_id = u.user_id
                ORDER BY t.transaction_date DESC"; // You can change this order as needed
                ...
                if ($result = $conn->query($sql)) {
                  if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                      $user_initials = strtoupper(substr($row['username'], 0, 2)); // Get first two letters of the username
                      $amount = $row['amount'];
                      $transaction_type = $row['transaction_type'] == 'income' ? 'text-success' : 'text-error'; // Green for income, red for expense
                      $date = date('M d, Y - h:i', strtotime($row['transaction_date'])); // Format the date
                ?>
                      ...
                      <div class="flex cursor-pointer items-center justify-between">
                        <div class="flex items-center space-x-3">
                          <div class="avatar">
                            <div class="rounded-full w-8 h-8 flex items-center justify-center bg-gray-300 text-white">
                              <span class="font-bold"><?php echo $user_initials; ?></span>
                            </div>
                          </div>
                          <div>
                            <p class="text-slate-700 line-clamp-1 dark:text-navy-100"><?php echo $row['username']; ?></p>
                            <p class="text-xs text-slate-400 line-clamp-1 dark:text-navy-200"><?php echo $date; ?></p>
                          </div>
                        </div>
                        <p class="font-medium <?php echo $transaction_type; ?>">$<?php echo number_format($amount, 2); ?></p>
                        <p class="text-xs text-slate-500"><?php echo $row['description']; ?></p> <!-- Description -->
                      </div>
        ...
        <div class="card col-span-12 px-4 pb-5 sm:px-5 lg:col-span-4">
                ...
                <div
                  ...
                  <ul>
                    ...
                    <li>
                      <a
                        href="#"
                        class="flex h-8 items-center px-3 pr-8 font-medium tracking-wide outline-none transition-all hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:hover:bg-navy-600 dark:hover:text-navy-100 dark:focus:bg-navy-600 dark:focus:text-navy-100">Another Action</a>
                    </li>
                    <li>
                      <a
                        href="#"
                        class="flex h-8 items-center px-3 pr-8 font-medium tracking-wide outline-none transition-all hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:hover:bg-navy-600 dark:hover:text-navy-100 dark:focus:bg-navy-600 dark:focus:text-navy-100">Something else</a>
                    </li>
                  </ul>
                  ...
                  <ul>
                    <li>
                      <a
                        href="#"
                        class="flex h-8 items-center px-3 pr-8 font-medium tracking-wide outline-none transition-all hover:bg-slate-100 hover:text-slate-800 focus:bg-slate-100 focus:text-slate-800 dark:hover:bg-navy-600 dark:hover:text-navy-100 dark:focus:bg-navy-600 dark:focus:text-navy-100">Separated Link</a>
                    </li>
                  </ul>
                </div>
              </div>
          ...
          </div><!-- Frontend: Sending Money Form -->
          <div class="mt-2 space-y-4">
            <label class="block">
              <span class="text-xs+">Pay to</span>
              <input id="receiver_account_number" class="form-input mt-1.5 h-9 w-full rounded-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:border-slate-400 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent" placeholder="Receiver's Account Number" type="text" />
            </label>

            <div>
              <span class="text-xs+">Amount</span>
              <div class="mt-1.5 flex h-9 -space-x-px">
                <select id="currency" class="form-select rounded-l-lg border border-slate-300 bg-white px-3 py-2 pr-9 hover:z-10 hover:border-slate-400 focus:z-10 focus:border-primary dark:border-navy-450 dark:bg-navy-700 dark:hover:border-navy-400 dark:focus:border-accent">
                  <option>$</option>
                  <option>£</option>
                  <option>€</option>
                </select>
                <input id="amount" class="form-input w-full rounded-r-lg border border-slate-300 bg-transparent px-3 py-2 placeholder:text-slate-400/70 hover:z-10 hover:border-slate-400 focus:z-10 focus:border-primary dark:border-navy-450 dark:hover:border-navy-400 dark:focus:border-accent" placeholder="Enter Amount" type="number" />
              </div>
            </div>
          </div>
          ...
          <div class="mt-2 flex justify-between">
            <p>Total:</p>
            <p id="total_amount" class="font-medium text-slate-700 dark:text-navy-100">$3</p>
          </div>

          <button id="send_money_button" class="btn mt-5 h-10 w-full bg-primary font-medium text-white hover:bg-primary-focus focus:bg-primary-focus active:bg-primary-focus/90 dark:bg-accent dark:hover:bg-accent-focus dark:focus:bg-accent-focus dark:active:bg-accent/90">
            Send Money
          </button>

          <script>
            // Commission calculation and total amount update on input change
            document.getElementById('amount').addEventListener('input', function() {
              const amount = parseFloat(document.getElementById('amount').value);
              const commissionRate = 3; // Fixed commission (can be dynamic based on percentage)
              ...
              if (!isNaN(amount) && amount > 0) {
                ...
                document.getElementById('commission').textContent = '$3';
                document.getElementById('total_amount').textContent = '$3';
              }
            });

            // Send money button click handler
            document.getElementById('send_money_button').addEventListener('click', function() {
              const receiverAccountNumber = document.getElementById('receiver_account_number').value;
              const amount = parseFloat(document.getElementById('amount').value);
              const totalAmount = amount + 3; // Include commission

              if (!receiverAccountNumber || isNaN(amount) || amount <= 0) {
